...
public class ViewField {
	private Point focus;
	private Tile[][] view;
	public static int width = (int) Math.ceil(Window.width/Tile.getWidth())+2;
	public static int height = (int) Math.ceil(Window.height/Tile.getWidth())+2;
	...
	public ViewField(Camera camera) {
		...
		topLeft = new Location((int) (Math.ceil(focus.getX()/Tile.getWidth())-ViewField.width/2), (int)(Math.ceil(focus.getY()/Tile.getWidth())-ViewField.height/2)+1);//aiobe, -56
		update(focus);
	...
	public void update(Point focus) {
		this.focus = focus;
		topLeft = new Location((int) (Math.ceil(focus.getX()/Tile.getWidth())-ViewField.width/2), (int)(Math.ceil(focus.getY()/Tile.getWidth())-ViewField.height/2)+1);//Verified math
	...
	public Tile getFocusTile() {
		return Tile.getTile(allTiles[(int) Math.ceil(focus.getX()/40)][(int) Math.ceil(focus.getY()/40)]);
	}
